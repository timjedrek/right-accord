const data = {
  "metaData": {
    "title": "Home Health Care Charlotte Park, FL | RIGHT ACCORD Private Duty Home Health Care",
    "ignoreTitleTemplate": true
  },
  "city": "Charlotte Park",
  "stateShort": "FL",
  "stateLong": "Florida",
  "slug": "charlotte-park-fl",
  "county": "Charlotte County",
  "description": "RIGHT ACCORD Home Health Care proudly serves Charlotte Park with compassionate, in-home care services. Whether your loved one needs hourly support or 24-hour specialized care, we tailor every plan to their unique needs.",
  "nearbyLocations": [
    "<a href=\"/locations/punta-gorda-fl\">Punta Gorda</a>",
    "<a href=\"/locations/solana-fl\">Solana</a>",
    "<a href=\"/locations/charlotte-harbor-fl\">Charlotte Harbor</a>",
    "<a href=\"/locations/port-charlotte-fl\">Port Charlotte</a>",
    "<a href=\"/locations/north-port-fl\">North Port</a>"
  ],
  "headerImages": [
    "/src/assets/images/right-accord-health-florida-HOMEPAGE_FIRSTPICTURE.jpg",
    "/src/assets/images/right-accord-health-florida-HOMEPAGE_ALZHAIMERS.jpg",
    "/src/assets/images/right-accord-health-florida-HOMEPAGE_PARKINSONS.jpg",
    "/src/assets/images/right-accord-health-florida-HOMEPAGE_AFTERSTROKE.jpg",
    "/src/assets/images/right-accord-health-florida-HOMEPAGE_HOSPICE.jpg"
  ],
  "headLines": [
    "In-Home Senior Care in Charlotte Park, FL",
    "Dependable Hourly & 24-Hour Caregivers",
    "Specialized Alzheimer\u2019s and Dementia Support",
    "Trusted Care from a Local, Nurse-Owned Agency",
    "Home Care That Feels Like Family"
  ],
  "keywords": [
    "Charlotte Park home health care",
    "in-home senior care Charlotte Park",
    "dementia care Charlotte Park FL",
    "private duty caregivers Charlotte Park",
    "elderly care Charlotte Park Florida",
    "companion care Charlotte Park",
    "Florida senior care services",
    "RIGHT ACCORD Charlotte Park"
  ]
};

export default { data };

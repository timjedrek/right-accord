const data = {
  "metaData": {
    "title": "Home Health Care Punta Gorda, FL | RIGHT ACCORD Private Duty Home Health Care",
    "ignoreTitleTemplate": true
  },
  "city": "Punta Gorda",
  "stateShort": "FL",
  "stateLong": "Florida",
  "slug": "punta-gorda-fl",
  "county": "Charlotte County",
  "description": "RIGHT ACCORD Home Health Care proudly serves Punta Gorda with compassionate, in-home care services. Whether your loved one needs hourly support or 24-hour specialized care, we tailor every plan to their unique needs.",
  "nearbyLocations": [
    "<a href=\"/locations/charlotte-park-fl\">Charlotte Park</a>",
    "<a href=\"/locations/solana-fl\">Solana</a>",
    "<a href=\"/locations/charlotte-harbor-fl\">Charlotte Harbor</a>",
    "<a href=\"/locations/port-charlotte-fl\">Port Charlotte</a>",
    "<a href=\"/locations/north-port-fl\">North Port</a>"
  ],
  "headerImages": [
    "/src/assets/images/right-accord-health-florida-HOMEPAGE_FIRSTPICTURE.jpg",
    "/src/assets/images/right-accord-health-florida-HOMEPAGE_ALZHAIMERS.jpg",
    "/src/assets/images/right-accord-health-florida-HOMEPAGE_PARKINSONS.jpg",
    "/src/assets/images/right-accord-health-florida-HOMEPAGE_AFTERSTROKE.jpg",
    "/src/assets/images/right-accord-health-florida-HOMEPAGE_HOSPICE.jpg"
  ],
  "headLines": [
    "In-Home Senior Care in Punta Gorda, FL",
    "Dependable Hourly & 24-Hour Caregivers",
    "Specialized Alzheimer\u2019s and Dementia Support",
    "Trusted Care from a Local, Nurse-Owned Agency",
    "Home Care That Feels Like Family"
  ],
  "keywords": [
    "Punta Gorda home health care",
    "in-home senior care Punta Gorda",
    "dementia care Punta Gorda FL",
    "private duty caregivers Punta Gorda",
    "elderly care Punta Gorda Florida",
    "companion care Punta Gorda",
    "Florida senior care services",
    "RIGHT ACCORD Punta Gorda"
  ]
};

export default { data };
